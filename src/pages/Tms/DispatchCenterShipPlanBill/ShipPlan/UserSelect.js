...
export default class UserSelect extends PureComponent {
  ...
  onFocusCallback = () => {
    ...
    if (this.state.value) {
      payload.searchKeyValues.codeName = JSON.parse(this.state.value).code
    }
    this.props.dispatch({
      type: 'workType/query',
      payload: payload
  ...
  render() {
    const { single, style,key } = this.props;
    const selectProps = {
      showSearch: true,
    ...
    return (
      <Select {...selectProps} onFocus={this.onFocusCallback} placeholder={this.props.placeholder} id='userforworktype' style={{width:'100%'}}>
        {this.buildOptions()}
      </Select>
    );
  }
