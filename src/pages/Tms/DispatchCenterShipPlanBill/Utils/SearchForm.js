...
export default class SearchForm extends PureComponent {
    ...
    drawButtonGroup = () => {
        ...
        return (
            <Col key='btnGroup' style={{ float: 'right',marginBottom:'1%'}}>
                <div>
                    {toggle == undefined && ''}
    ...
    drawRows = () => {
        ...
        for (let i = 0; i < cols.length; i++) {
            let col = cols[i];
            if (currentRowCols.length < (this.state!=null&&this.state.showColCount?this.state.showColCount:3)) {
    ...
    render() {
        ...
        return (
            <SearchPanel>
                <Form  {...formItemLayout}  className={styles.searchForm} onSubmit={this.handlerSearch} autoComplete="off">
                    {this.drawRows()}
                </Form>
            </SearchPanel>
        );
