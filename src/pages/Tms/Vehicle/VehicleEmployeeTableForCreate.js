...
export default class VehicleEmployeeTableForCreate extends React.Component {
  constructor(props) {
    super(props);
  ...
  onSave = (record) => {
    if (record.empUuid && record.workType) {
      ...
      this.props.dispatch({
        type: 'vehicle/saveEmployeeForCreat',
        payload: record
      })
    }
  }
  ...
  onRemove = (record) => {
    ...
    if (record.vehicleUuid) {
      ...
      this.props.dispatch({
        type: 'vehicle/removeEmpForCreat',
        payload: {
          params,
    ...
    let columns = [
      {
        ...
        render: record => {
          if (record && record.vehicleUuid)
            ...
            style={{ width: '100%' }}
            single={true}
            onChange={this.onCodeChange.bind(this, record)}
    ...
    return (
      <div>
        <ViewPanel title={VehicleLocale.empInfo}>
          {loginOrg().type === orgType.carrier.name ?
            <Table
