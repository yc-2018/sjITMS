...
export default class CarrierViewPage extends ViewPage {
	constructor(props) {
      ...
      entityCode: props.carrier.entityCode,
      entityUuid: props.carrier.entityUuid,
      entity: {}
		}
	}
	...
	refresh (entityCode) {
	  ...
	  if(entityCode){
      this.props.dispatch({
        type: 'carrier/getByCode',
        payload: {
          code: entityCode
        },
        callback:(response)=>{
          if(!response || !response.data || !response.data.uuid){
            message.error("指定承运商不存在。")
            this.onCancel()
          }else{
            this.setState({
	...
	componentWillReceiveProps(nextProps) {
		...
		if (entity) {
			// if (nextProps.carrier.entityUuid !== entity.uuid) {
			// 	this.refresh();
			// }
			this.setState({
				entity: entity,
	...
	drawBasicInfoTab = () => {
		...
		let basicItems = [{
			...
			value: entity.address && JSON.stringify(entity.address) !== "{}" ? addressToStr(entity.address) : ''
		}, {
