...
import styles from './FeeType.less';
const FormItem = Form.Item;
const Search = Input.Search;

const taskTypeMap = { enable: 'enable', disable: 'disable' };

@connect(({ feeType, loading }) => ({
  feeType,
  loading: loading.models.feeType,
}))
class FeeType extends PureComponent {
  state = {
    record: {},
    data:{},
    pageFilter: {
  ...
  componentWillReceiveProps(nextProps){
    if(this.props.feeType&&this.props.feeType.data!=nextProps.feeType.data){
      this.setState({
        data:nextProps.feeType.data
      })
    }
  }
  ...
  handleAddType = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      let type = 'feeType/onSave';
      if (record.uuid) {
        type = 'feeType/onModify';
  ...
  render() {
    ...
    const actionBtn = (
      <Fragment>
        ...
        <Button onClick={() => this.backToBefore(this.props.feeType)}>
          返回
        </Button>
      </Fragment>
    );
      ...
      <PageHeaderWrapper>
        <Page>
          ...
          <Search
            ...
            placeholder={placeholderLocale('类型名称')}
          ...
          <div className={styles.standardTable}>
            <StandardTable
              ...
              comId={'feeType.unCommonSearch.table'}
            />
          </div>
        </Page>
        <Modal
              ...
              >
                {
                  getFieldDecorator('type', {
                    rules: [{ required: true, message: notNullLocale('类型') }, {
                      max: 30, message: tooLongLocale('类型', 30),
                    }],
                    initialValue: record.type,
                  })(
                    <Input placeholder={placeholderLocale(placeholderName+'的类型')} autoFocus/>
                  )
                }
              </FormItem>
              <FormItem
              ...
              >
                {
                  getFieldDecorator('name', {
                    ...
                    <Input placeholder={placeholderLocale(placeholderName+'的名称')} autoFocus/>
                  )
                }
              </FormItem>
              <FormItem
              ...
              >
                {
                  getFieldDecorator('note', {
                    rules: [{
                      max: 255, message: tooLongLocale('备注'),
                    }],
                    initialValue: record.note,
                  })(
                    <TextArea placeholder={placeholderLocale('备注')} />
                  )
                }
              </FormItem>
            </Form>
...
export default Form.create()(FeeType);
