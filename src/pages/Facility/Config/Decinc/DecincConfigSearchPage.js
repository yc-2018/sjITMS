...
import DecincConfigSearchForm from './DecincConfigSearchForm';

@connect(({ decincConfig, loading }) => ({
    decincConfig,
    loading: loading.models.decincConfig,
}))
export default class DecincConfigSearchPage extends ConfigSearchPage {
    ...
    getConfigType = (configType) => {
        ...
        if ("DEC" === configType) {
            caption = decincConfigLocale.decincConfigLoss;
            
        }
        if ("INC" === configType) {
            caption = decincConfigLocale.decincConfigSpill;
        }
        return caption;
    }

    drawSearchPanel = () => {
        return <DecincConfigSearchForm filterValue={this.state.pageFilter.searchKeyValues} refresh={this.onSearch} />;
    }
