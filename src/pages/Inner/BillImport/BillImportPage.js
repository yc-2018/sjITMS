    ...
    return (
      <div>
        ...
        <BillImportSearchForm
          ...
          mouldString={
            this.state.entity && this.state.entity.code ? (
              convertCodeName(this.state.entity)
            ) : (
              <Empty />
            )
          }
      ...
      <Dragger {...props}>
        <div>
          ...
          <p className="ant-upload-text">
            {formatMessage({ id: 'common.excelImport.select.uploadText' })}
          </p>
          <br />
          <br />
          <p className={styles.typeTips}>
            {formatMessage({ id: 'common.excelImport.select.uploadTips' })}
          </p>
        </div>
      </Dragger>
    );
  ...
  drawContent = () => {
    ...
    if (billImportResult && billImportResult.success) {
      ...
      billImportResult.billNumbers.forEach((billNumber, index) => {
        ...
        else billNumberStr = billNumberStr + billNumber + '、';
      });
      noteItems.push({
        ...
        <div style={{ marginTop: '20px' }}>
          {!result.errorDownloadUrl &&
            result.successCount > 0 && (
              <div className={styles2.report}>
                <span>
                  {formatMessage({ id: 'common.excelImport.result.tips.importSituation' })}
                </span>
                <div className={styles2.detail}>
                  <div style={{ marginTop: '20px' }}>
                    <Icon type="check" style={{ color: 'green' }} /> {'全部导入成功'}
                  </div>
                </div>
              </div>
            )}
          {!result.errorDownloadUrl &&
            ...
            result.errorCount === 0 && (
              <div className={styles2.report}>
                ...
                <div className={styles2.detail}>
                  <div style={{ marginTop: '20px' }}>
                    <Icon type="close" style={{ color: 'red' }} /> {'全部导入失败'}
                  </div>
                </div>
              </div>
            )}
          ...
          {result.errorDownloadUrl &&
            result.errorCount > 0 && (
              <Button
                ...
                onClick={() => this.downloadFile(result.errorDownloadUrl, false)}
              >
