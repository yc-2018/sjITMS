...
import InfinityTable from '@/pages/Component/Form/InfinityTable';
...
export default class ItemEditTable extends PureComponent {
  ...
  buildColumns = () => {
    const { columns, noLine } = this.props;
    if (columns[columns.length - 1].key === 'action') {
      return columns;
    }

    if (!noLine && !columns.find(item => item.key === 'line')) {
      columns.unshift(
        {
  ...
  rowSelection = {
    columnWidth: 40,
    fixed: this.props.fixed ? this.props.fixed : false,
    ...
    return (
      <div id="editTable">
        ...
        {/* 
          ...
          /> : */}
        <Table
          rowSelection={this.props.noSelection ? false : this.rowSelection}
