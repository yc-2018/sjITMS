...
import { Modal, InputNumber, Button, message,Tabs,Row,Col, Form ,Select,Icon,Input} from 'antd';
import { loginOrg, loginCompany } from '@/utils/LoginContext';
import { convertCodeName } from '@/utils/utils';
import Empty from '@/pages/Component/Form/Empty';
import { accAdd } from '@/utils/QpcStrUtil';
import { modify, query,getFeeType ,saveOrUpdateFee} from '@/services/sjtms/otherFee';
...
export default class OtherFeeModal extends Component {
  ...
  callback =(key)=> {
    ...
    if(key==1){
      ...
      this.getFeeType();
      this.setState({isView:false,defaultActiveKey:'2',amount:'',feeType:'',feeUuid:null})
    ...
    return <Modal
      ...
      <TabPane tab="费用录入" key="2">
        ...
        <Col span={6}>
            ...
            <Select style={{ width: 120 }} onBlur={(e)=>{this.setState({feeType:e})}} >
        ...
        <Form.Item label={"金额"} labelCol={{span:12}}>
            ...
            <Input  name='amount' style={{ width: 120  ,float:'left'}} onBlur={(e)=>{this.setState({amount:e.target.value})}}></Input>
          )}
            </Form.Item>
        </Col>
        </Form>
