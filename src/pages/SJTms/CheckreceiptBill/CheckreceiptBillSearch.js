...
import styless from './transportOrder.less';
...
export default class CheckreceiptBillSearch extends QuickFormSearchPage {
  ...
  changeState = async () => {
    //增加是否合并配置
    let res = await getConfigDataByParams('checkreceiptbill', loginOrg().uuid, 'isMerge');
    let isMerge = res.data[0].isMerge == 1 ? true : false;
    this.setState({ isMerge });
  }; //扩展state
    ...
    if (e.column.fieldName == 'RECEIPTED') {
          ...
          onClick={v => {
            ...
            if (this.state.isMerge && e.record.detail) {
              e.record.detail.map(d => {
                d.RECEIPTED = v.target.checked ? 1 : 0;
              });
            }
            if (v.target.checked) {
              e.record.DEALMETHOD = '';
        ...
        >
          <Button
            onClick={v => {
              ...
              if (this.state.isMerge && e.record.detail) {
                e.record.detail.map(d => {
                  d.RECEIPTED = e.record.RECEIPTED;
                });
              }
              if (e.record.RECEIPTED == 1) {
                ...
                if (this.state.isMerge && e.record.detail) {
                  e.record.detail.map(d => {
                    d.DEALMETHOD = e.record.DEALMETHOD;
                  });
                }
              }
            }}
      ...
      e.component = (
        <Input
          ...
          placeholder="请输入备注"
          onChange={v => {
            e.record.NOTE = v.target.value;
            if (this.state.isMerge && e.record.detail) {
              e.record.detail.map(d => {
                d.NOTE = v.target.value;
              });
            }
          }}
        />
