 ...
 * @LastEditTime: 2024-03-08 16:11:04
 * @Description: 打印拣货记录
          ...
          <tr>
            <th colspan={8} style={{ border: 0 }}>
              <div style={{ textAlign: 'left', fontWeight: 'normal', height: 20 }}>
                ...
                <div style={{ float: 'left', width: '32%' }}>申请货品:{detail.articlecode}    申请数量:{detail.qty}</div>
              </div>
            </th>
          </tr>
          ...
          {records ? (
            records.map((item) => {
              return (
                ...
                >
                  <td width={100}>{item.cartonno}</td>
                  <td width={50}>{item.fmlocation}</td>
                  <td width={50}>{item.sku}</td>
                  <td width={50}>{item.consigneeid}</td>
                  <td width={50}>{item.fmqtyEach}</td>
                  <td width={50}>{item.qtyordered}</td>
                  <td width={120}>{item.edittime}</td>
        ...
        <tfoot>
          <tr style={{ height: 25, borderColor: "#011627" }}>
            ...
            <td style={centerStyle}>
              {records ? (Math.round(sumBy(records, x => x.fmqtyEach) * 100) / 100) : 0}
            </td>
            <td style={centerStyle}>
