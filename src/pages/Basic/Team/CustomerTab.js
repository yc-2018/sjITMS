...
export default class CustomerTab extends PureComponent {
  ...
  onSave = (data) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      dispatch({
        type: 'team/modifyClassGroupCustomer',
        payload: payload,
        callback: response => {
  ...
  onRemove = (record) => {
    ...
    return new Promise(function (resolve, reject) {
      ...
      dispatch({
        type: 'team/deleteClassGroupCustomer',
        payload: record.uuid,
        callback: response => {
          if (response && response.success) {
            resolve({
  ...
  onFieldChange = (fieldName, e, target) => {
    if (fieldName === 'customerCode') {
      target.customer = JSON.parse(e);
    }
    target.customerType = target.customer.type;
  }
  ...
  render() {
    const columns = [{
      ...
      componentName: 'TeamCustomerSelect',
      placeholder: '选择客户',
      dataIndex: 'customerCode',
      key: 'uuid',
      render: loginOrg().type !== orgType.dispatchCenter.name ? (text, record) => {
        return record.customer? record.customer.code : '';
      } : ''
    }, {
