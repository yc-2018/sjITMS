    ...
    if (target) {
      if (!target.editable) {
        ...
        if (this.props.checkEditOrRemove) {
          this.props.checkEditOrRemove(target).then(result => {
            if (!result.success) {
    ...
    if (c.componentName === 'Input') {
      return (
        <Input
          value={text}
          disabled={
            false == record.editQpcStr && ('qpcStr' === c.dataIndex || 'barcode' === c.dataIndex)
          }
          autoFocus={index === 0}
      ...
      return (
        <OrgSelect
          ...
          single
          placeholder={placeholderChooseLocale(
            c.orgTypePlaceholder ? c.orgTypePlaceholder : articleLocale.vendor
          )}
          value={text}
