 ...
 */
export default class ConfigSearchForm extends Component {
    componentDidUpdate() {
        if (this.drawRows()) {
    ...
    drawButtonGroup = () => {
        ...
        return (
            <Col span={8} key='btnGroup' style={{ float: 'right' }}>
    ...
    buildFormItems = () => {
        ...
        for (let i = 0; i < cols.length; i = i + 3) {
        ...
        if (firstLabelLength === 3) {
            firstColLabelSpan = firstColLabelSpan + addedSpan;
        }

        if (firstLabelLength === 4) {
            firstColLabelSpan = firstColLabelSpan + addedSpan * 2;
        }

        for (let i = 1; i < cols.length; i = i + 3) {
            if (!cols[i].props.label) {
                continue;
            }
            let thisLength = cols[i].props.label.length;
        ...
        if (twoLabelLength === 3) {
            twoColLabelSpan = twoColLabelSpan + addedSpan;
        }

        if (twoLabelLength === 4) {
            twoColLabelSpan = twoColLabelSpan + addedSpan * 2;
        }

        for (let i = 2; i < cols.length; i = i + 3) {
            if (!cols[i].props.label) {
                continue;
            }
            let thisLength = cols[i].props.label.length;
        ...
        if (threeLabelLength === 3) {
            threeColLabelSpan = threeColLabelSpan + addedSpan;
        }

        if (threeLabelLength === 4) {
            threeColLabelSpan = threeColLabelSpan + addedSpan * 2;
        }
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            formItems.push(
                <Col key={cols[i].props.label} span={8}>
