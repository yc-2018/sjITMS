 ...
 * @LastEditTime: 2022-02-17 09:15:08
 * @version: 1.0
 */
...
export default class QuickCreate extends QuickCreatePage {
    ...
    handleChange = (value, tableName, dbFieldName, fieldShowType, line, fieldExtendJsons, onlFormFields) => {
        if (line == undefined) {
            line = 0;
        } else if (line != 0) {
            line -= 1;
        }
        console.log('value', value, 'tableName', tableName, 'dbFieldName', dbFieldName, 'fieldShowType', fieldShowType,
            'fieldExtendJson', fieldExtendJsons, 'onlFormFields', onlFormFields);
        ...
        if (fieldExtendJsons.childComponent) {
            const onlFormField = onlFormFields.find(data => data.dbFieldName == fieldExtendJsons.childComponent);
