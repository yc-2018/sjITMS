...
export default class ViewTablePanel extends PureComponent {
  ...
  getColumnSearchProps = (dataIndex, oldRender) => {
    // filter or sort
    const isCurrent =
      this.state.searchedColumn === dataIndex && (this.state.sort || this.state.searchText);
      ...
      filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div className={styles.filterDropDown}>
          ...
          <a
            ...
            style={ascStyle}
            onClick={() => this.sortData(dataIndex, true, false, confirm)}
          >
            <div>
              <IconFont
                type="positiveorder"
                style={{
                  fontSize: '16px',
                  color: ascStyle.iconColor,
                  position: 'relative',
                  top: 1,
                }}
              />
              &nbsp;&nbsp;
              <span style={{ color: '#354052' }}>正排序</span>
            </div>
          </a>
          <a
            className={styles.sortFilter}
            style={descStyle}
            onClick={() => this.sortData(dataIndex, false, false, confirm)}
          >
            <div>
              <IconFont
                type="reverseorder"
                style={{
                  fontSize: '16px',
                  color: descStyle.iconColor,
                  position: 'relative',
                  top: 1,
                }}
              />
              &nbsp;&nbsp;
              <span style={{ color: '#354052' }}>倒排序</span>
            </div>
          </a>
        </div>
      ),
      filterIcon: filtered =>
        this.state.hoverColumn === dataIndex || this.state.dropdownColumn === dataIndex ? (
  ...
  render() {
    ...
    return (
      <div
        className={styles.viewTablePanelWrapper}
                ...
                <ToolbarPanel>
                  <div className={style.toolbarIcon} style={{ float: 'left' }}>
                    {this.props.drawBatchButton &&
                      this.props.drawBatchButton(this.state.selectedRowKeys)}
                    &nbsp;&nbsp;&nbsp;
                  </div>
                </ToolbarPanel>
              )}
