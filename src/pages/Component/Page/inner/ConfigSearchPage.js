...
export default class ConfigSearchPage extends Component {
  ...
  constructor(props) {
    ...
    if (sessionStorage.getItem('config.selectedKey') && sessionStorage.getItem(sessionStorage.getItem('config.selectedKey'))) {
      queryFilter = JSON.parse(sessionStorage.getItem(sessionStorage.getItem('config.selectedKey')));
    }
  ...
  render() {
    ...
    if (sessionStorage.getItem('config.selectedKey')) {
      sessionStorage.setItem(sessionStorage.getItem('config.selectedKey'), JSON.stringify(tableFilter));
    }
    const tableLoading = {
          ...
          }
          {
            !this.state.hideLogTab ?
              < TabPane tab={formatMessage({ id: 'company.detail.tab.operateInfo' })} key="2">
                <EntityLogTab
                  entityUuid={this.state.onlyEntityUuid ? this.state.logCaption : loginOrg().uuid + this.state.logCaption}
