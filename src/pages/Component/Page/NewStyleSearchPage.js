...
import NewPageHeaderWrapper from '@/components/NewPageHeaderWrapper';
import Page from '@/pages/Component/Page/inner/NewStylePage';
import NewNavigatorPanel from '@/pages/Component/Page/inner/NewNavigatorPanel';
import ConfirmProgress from '@/pages/Component/Progress/ConfirmProgress';
import NewStandardTable from '@/components/NewStandardTable';
import LoadingIcon from '@/pages/Component/Loading/LoadingIcon';
import NewToolbarPanel from '@/pages/Component/Page/inner/NewToolbarPanel';
import { message,Modal } from 'antd';
import { formatMessage } from 'umi/locale';
import { getActiveKey, getPageFilter, setPageFilter } from '@/utils/NewLoginContext';
import { routerRedux } from 'dva/router';
import FreshPageHeaderWrapper from '@/components/NewPageHeaderWrapper/FullScreenPageWrapper';
 ...
 */
export default class NewStyleSearchPage extends Component {
  constructor(props) {
    super(props);
...
// 确认框确定按钮 -- onOk
  ...
  drawToolbar = () => {
    if (this.drawToolbarPanel) {
      return (<NewToolbarPanel>{this.drawToolbarPanel()}</NewToolbarPanel>);
    } else {
      return '';
    }
  };
  ...
  drawPage = () => {
    ...
    return <Page withCollect={true} pathname={this.props.pathname}>
      <NewNavigatorPanel canFullScreen={this.state.canFullScreen} title={this.state.title}
                      action={this.drawActionButton ? this.drawActionButton() : ''}/>
      {this.drawSearchPanel ? this.drawSearchPanel() : ''}
      {this.drawToolbar()}
      {this.drawToolbarTwo()}
      {!this.state.noTable ? <NewStandardTable
  ...
  render() {
    let ret = this.state.canFullScreen ?
      ...
      : <NewPageHeaderWrapper>{this.drawPage()}{this.drawProgress()}</NewPageHeaderWrapper>;
