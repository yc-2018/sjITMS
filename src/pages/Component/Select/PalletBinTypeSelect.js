 ...
 */
@connect(({ palletBinType }) => ({
  palletBinType,
}))
export default class PalletBinTypeSelect extends PureComponent {
  ...
  componentDidMount() {
    this.props.dispatch({
      type: 'palletBinType/getByCompanyAndDc',
      payload: {
  ...
  buildOptions = () => {
    ...
    if (this.props.hasAll) {
      options.push(<Select.Option key="palletBinTypeAll" value=''> 全部 </Select.Option>);
    }

    let data = this.props.palletBinType.data.list;
