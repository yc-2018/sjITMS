...
export default class OwnerConfigSelect extends PureComponent {
    ...
    if (this.props.hasAll) {
      ...
      Array.isArray(data) && data.forEach(function (dg) {
        if (dg.companyUuid === loginCompany().uuid) {
          options.push(
            <Select.Option key={dg.uuid} value={dg.code}> {'[' + dg.code + ']' + dg.name} </Select.Option>
          );
        }
      });
    } else {
      Array.isArray(data) && data.forEach(function (dg) {
        ...
        if(dg.companyUuid===loginCompany().uuid){
          options.push(
            <Select.Option key={dg.uuid} disabled={off} value={dg.code}> {'[' + dg.code + ']' + dg.name} </Select.Option>
          );
        }
      });
    }
