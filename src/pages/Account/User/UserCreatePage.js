...
export default class UserCreatePage extends CreatePage {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.user.entity &&
      this.props.user.entityUuid &&
      nextProps.user.entity.uuid == this.props.user.entityUuid
    ) {
      this.setState({
  ...
  fetchUserByUuid = () => {
    ...
    const {
      tempRoleResourceMap,
      dcTempRoleResourceMap,
      storeTempRoleResourceMap,
      vendorTempRoleResourceMap,
      carrierTempRoleResourceMap,
      dispatchCenterTempRoleResourceMap,
    } = this.state;
  ...
  setDeSelectRoleResources = role => {
    ...
    if (role.uuid) {
      ...
      let dispatchCenterChecks = this.deleteRoleResource(
        tempRoleResourceMap,
        checkedResourceKeys,
        role
      );
  ...
  deleteRoleResource = (resourceMap, checkedResourceKeys, role) => {
    if (!resourceMap || !checkedResourceKeys || !role) return undefined;
  ...
  drawFormItems = () => {
    ...
    if (orgTypes.indexOf(orgType.company.name) !== -1) {
      customOptions.push({
        ...
        caption:
          '[' + loginOrg().code + ']' + loginOrg().name + ' ' + getOrgCaption(loginOrg().type),
      });
    }
    ...
    let cols = [
      ...
      <CFormItem key="phone" label={userLocale.phone}>
        {getFieldDecorator('phone', {
          rules: [{ pattern: userLocale.phonePattern, message: userLocale.phonePatternMessage }],
      ...
      <CFormItem key="roleUuids" label={userLocale.roles}>
        {getFieldDecorator('roleUuids', {
          rules: [{ required: true, message: notNullLocale(userLocale.roles) }],
          initialValue: entity ? roleInitialValues : [],
        })(
          <RoleSelect
            placeholder={placeholderChooseLocale(userLocale.roles)}
            mode="multiple"
      ...
      loginOrg().type === 'COMPANY' && (
        <CFormItem key="orgUuids" label={userLocale.orgs} labelSpan={4} span={12}>
          {getFieldDecorator('orgUuids', {
            rules: [{ required: true, message: notNullLocale(userLocale.orgs) }],
