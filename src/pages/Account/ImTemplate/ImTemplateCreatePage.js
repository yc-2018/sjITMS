...
Object.keys(ImportTemplateType).forEach(function(key) {
  templateOptions.push(
    <Select.Option value={ImportTemplateType[key].name} key={ImportTemplateType[key].name}>
      {ImportTemplateType[key].caption}
    </Select.Option>
  );
});
...
class ImTemplateCreatePage extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.selectedImportTemplate &&
      nextProps.selectedImportTemplate != this.props.selectedImportTemplate
    ) {
      this.setState({
    ...
    if (
      nextProps.selectedImportTemplate == undefined &&
  ...
  handleChange = info => {
    ...
    if (info.file.status === 'done') {
      ...
      let data = info.file.response.data?.split(',');
      // attachments[info.file.name] = info.file.response.data;
      attachments[info.file.name] = data[0];
      // this.getPath(attachments[info.file.name]);
      this.state.entity.path = data[1];
      this.setState({
        entity: this.state.entity,
        path: data[1],
      });
      ...
      <div>
        <div style={{ marginTop: '-30px' }} className={styles.navigatorPanelWrapper}>
          <span className={styles.title}>
            {selectedImportTemplate && entity ? entity.name : imTemplateLocale.createTemp}
          </span>
        </div>
          ...
          <Form onSubmit={this.handleSave} {...formItemLayout}>
            <FormItem label={imTemplateLocale.tempName}>
              {form.getFieldDecorator('name')(
                <span>
                  {entity.name ? entity.name : selectedType.caption + imTemplateLocale.title}
                </span>
              )}
            </FormItem>
            <FormItem label={imTemplateLocale.tempType}>
              {form.getFieldDecorator('type')(
                <span>
                  {entity.type ? ImportTemplateType[entity.type].caption : selectedType.caption}
                </span>
              )}
            </FormItem>
            <FormItem label={imTemplateLocale.title} key={selectedType.name + i}>
              {form.getFieldDecorator('upload', {
                rules: [
                  {
                    required: true,
                    message: notNullLocale(imTemplateLocale.title),
                  },
                ],
              })(
                <Upload {...props}>
