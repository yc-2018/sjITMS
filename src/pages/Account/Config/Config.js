...
export default class Config extends SiderPage {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      openKeys: loginOrg().type == 'HEADING' ? [configLocale.billDailyKnotsConfig.billConfig.key] : [configLocale.interfaceConfig.key],
      selectedKeys: loginOrg().type == 'HEADING' ? [configLocale.billDailyKnotsConfig.billConfig.key] : (
        loginOrg().type == 'DISPATCH_CENTER' ? [configLocale.timeIntervalConfig.timeIntervalConfig.key] :[configLocale.interfaceConfig.interfaceConfig.key]
      ),
      siderStyle: {
        height: '650px',
        overflow: 'auto'
  ...
  drawSider = () => {
    ...
    return (
      ...
      >
        {/* 海鼎端 */}
        {
          loginOrg().type === 'HEADING' ? <SubMenu key={configLocale.billDailyKnotsConfig.billConfig.key} title={<span><Icon type="folder" /><span>{configLocale.billDailyKnotsConfig.billConfig.name}</span></span>}>
        ...
        }
        {
          loginOrg().type === 'HEADING' ?
            <SubMenu key={configLocale.billDailyKnotsConfig.dailyKnotsConfig.key} title={<span><Icon type="folder" /><span>{configLocale.billDailyKnotsConfig.dailyKnotsConfig.name}</span></span>}>
        ...
        {loginOrg().type === 'COMPANY' ?
          <SubMenu key={configLocale.tmsConfig.key} title={<span><Icon type="folder" /><span>{configLocale.tmsConfig.name}</span></span>}>
        ...
        {loginOrg().type === 'COMPANY' ?
          <SubMenu key={configLocale.sccConfig.key} title={<span><Icon type="folder" /><span>{configLocale.sccConfig.name}</span></span>}>
            <Menu.Item key={configLocale.sccConfig.noticeConfig.key}>
              {configLocale.sccConfig.noticeConfig.name}
            </Menu.Item>
          </SubMenu> : null}
        {loginOrg().type === 'COMPANY' ?
          <SubMenu key={configLocale.stockBatchConfig.key} title={<span><Icon type="folder" /><span>{configLocale.stockBatchConfig.name}</span></span>}>
        ...
        {loginOrg().type === 'DISPATCH_CENTER'? 
          <SubMenu key={configLocale.timeIntervalConfig.key} title={<span><Icon type="folder" /><span>{configLocale.timeIntervalConfig.name}</span></span>}>
