...
export default class AlcNtcBillNumberSelect extends PureComponent {
	...
	buildOptions = () => {
		...
		Array.isArray(data) && data.forEach(function (alcNtc) {
			options.push(
				<Select.Option
          key={alcNtc.billNumber}
          value={alcNtc.sourceBillNumber + alcNtc.billNumber}
          title={'[' + alcNtc.sourceBillNumber + ']' + alcNtc.billNumber + '[' + alcNtc.store.code + ']' + alcNtc.store.name}>
          {'[' + alcNtc.sourceBillNumber + ']' + alcNtc.billNumber + '[' + alcNtc.store.code + ']' + alcNtc.store.name}
				</Select.Option>
			);
		});
		return options;
	...
	onChange = (e, selectValue) => {
		...
		if (this.props.onChange)
			this.props.onChange(e, selectValue);
	}
