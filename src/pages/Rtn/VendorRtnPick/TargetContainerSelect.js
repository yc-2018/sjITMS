...
export default class TargetContainerSelect extends PureComponent {
  ...
  buildOptions = () => {
    const { defaultBarcode } = this.props;

    let options = [];
    if (defaultBarcode)
      options.push(<Select.Option key={defaultBarcode} value={defaultBarcode}>
        {defaultBarcode}
      </Select.Option>)

    this.state.value && this.props.container.data && Array.isArray(this.props.container.data.list)
      && this.props.container.data.list.forEach(function (dg) {
        options.push(
