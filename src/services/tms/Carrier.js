...
export async function online(params) {
  ...
  return request(`/iwms-basic/basic/carrier/${uuid}/online?companyUuid=${loginCompany().uuid}&version=${params.version}`, {
    method: 'POST'
  });
}
...
export async function offline(params) {
  const uuid = params.uuid;
  return request(`/iwms-basic/basic/carrier/${uuid}/offline?companyUuid=${loginCompany().uuid}&version=${params.version}`, {
    method: 'POST'
  });
}
...
export async function getByCompanyUuidAndCode(param) {
  return request(`/iwms-basic/basic/carrier/${param.code}/${loginCompany().uuid}`);
}
...
export async function getByCompanyUuid(payload) {
  return request(`/iwms-basic/basic/carrier/bycompanyuuid?${stringify(payload)}`);
}
