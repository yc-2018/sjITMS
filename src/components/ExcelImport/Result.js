        ...
        if (response && response.success) {
          ...
          if (data) {
            ...
            if (data.successCount == 0 && data.errorCount > 0) {
              message.error(formatMessage({ id: 'common.excelImport.result.message.error.import' }));
          ...
          message.error(formatMessage({ id: 'common.excelImport.result.message.error.import' }) + response.message);
        }
        this.setState({
          uploading: false,
        })
      ...
      <div className={styles.result}>
        {uploading ?
          <div className={styles.uploading}>
            <Icon className={styles.icon} type="loading" style={{color: defaultSettings.primaryColor, fontSize: '48px' }} spin />
            <span>{formatMessage({ id: 'common.excelImport.result.tips.importing' })}</span>
          </div>
          : <div className={styles.detail}>
            <div className={styles.fileName}>
              <Icon type="file-excel" style={{color: defaultSettings.primaryColor, fontSize: '24px' }} theme="filled" />
              <span>{fileName}</span>
            </div>
            <div className={styles.report}>
              ...
              <div className={styles.detail}>
                <span>
                  <Icon type="check" style={{color: 'green' }} />  {formatMessage({ id: 'common.excelImport.result.tips.successed' })} {result.successCount}
                  &nbsp;{formatMessage({ id: 'common.excelImport.result.tips.record' })}
                </span><br/>
                <span>
                  <Icon type="close" style={{color: 'red' }} />  {formatMessage({ id: 'common.excelImport.result.tips.failed' })} {result.errorCount}
                  &nbsp;{formatMessage({ id: 'common.excelImport.result.tips.record' })}
                  </span>
              </div>
            </div>
            ...
            <div className={styles.operate}>
              ...
              <Button type="primary" ghost style={{display: 'block'}} onClick={() => downloadFile(result.errorDownloadUrl)}>
