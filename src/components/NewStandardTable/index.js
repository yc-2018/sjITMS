...
class StandardTable extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      pageSize: props.comId && props.comId.indexOf('search') > -1 ? sessionStorage.getItem("searchPageLine") :
          ...
          return {
            ...
            onMouseEnter: event => {
              ...
              if (clientWidth < scrollWidth) {
                ...
                ReactDOM.render(
                  <Tooltip placement="topLeft" title={targetEl.innerText}><span id={'hastooltip'}
                                                                                dangerouslySetInnerHTML={html}></span></Tooltip>,
                  targetEl,
                );
              }
            },
  ...
  settingColumns = [
    {
      ...
      render: (text, record, index) => {
        return (<Checkbox style={{ paddingLeft: 12 }} id={record.value} value={record.value} checked={record.checked}
    ...
    },
    {
      ...
      render: (text, option, index) => {
        return <span
          style={{ width: '100px', paddingLeft: 4, fontSize: '12px', fontWeight: 400 }}>{option.value}</span>;
      },
    },
    {
      title: 'hover',
  ...
  handleOK = () => {
    ...
    // 设置最后一列
    if (columns[columns.length - 1].title === commonLocale.operateLocale && !this.props.noActionCol) {
      newColumns.push(columns[columns.length - 1]);
    }
  ...
  isFixedEdge(column) {
    return column.key === 'action' || column.key === 'line' || column.key === 'billNumber'
      || column.title === commonLocale.operateLocale || column.key === commonLocale.lineLocal;
  }
  ...
  renderSettingDrowDown = (settingIcon) => {
    const menuItems = [];
    menuItems.push(<div style={{ padding: 12, paddingTop: 4, paddingBottom: 22 }}>
      <span style={{ float: 'left', fontWeight: 500 }}>列展示</span>
      <span style={{ float: 'right' }}>
        <a onClick={() => this.handleResetSetting()}>
          重置
        </a>
      </span>
    </div>);
    ...
    return <Dropdown overlay={menu} placement="bottomRight" overlayClassName={styles.settingDropdown}
                     trigger={['click']} arrow>
      {settingIcon}
    </Dropdown>;
  };
  ...
  render() {
    const { selectedRowKeys, needTotalList, columns, settingModalVisible, oriColumnLen } = this.state;
    ...
    if (totalWidth > tableWidth) {
      ...
      for (let idx = columns.length - 1; idx >= 0; idx--) {
        ...
        let newWidth = (idx === columns.length - 1 && columns.length < SHOW_THRESH_HOLD) ? this.strip(newColumns[idx].width * expandRatio) - 10 : this.strip(newColumns[idx].width * expandRatio);
    ...
    let settingIcon = <div className={styles.setting} onClick={() => this.handleSettingModalVisible(true)}>
      ...
      <div className={styles.standardTable}>
        {
          ((oriColumnLen >= SHOW_THRESH_HOLD && !noSettingColumns) || hasSettingColumns) ?
            <div style={style}>
              {
                this.renderSettingDrowDown(settingIcon)}
            </div> : null
        }
        <div id={this.state.settingKey} style={{ borderBottom: '1px solid transparent !important' }}></div>
        <Table
          className={this.props.tableClassName}
          id={this.state.key}
          rowKey={rowKey || 'key'}
          rowSelection={this.props.unShowRow ? undefined : (this.props.rowSelection ? this.props.rowSelection : rowSelection)}
          dataSource={showList}
          size={this.props.size ? this.props.size : 'middle'}
          pagination={paginationProps}
          onChange={this.handleTableChange}
          rowClassName={this.props.newRowClassName ? this.props.newRowClassName :
          ...
          onRow={this.props.onRow ? this.props.onRow : (this.props.unShowRow ? (record, index) => {
            return {
              index,
            ...
            return {
              index,
              moveRow: (this.props.canDrag && this.props.moveRow) ? this.props.moveRow : this.moveRow,
