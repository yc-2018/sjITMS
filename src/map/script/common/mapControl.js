...
import pointSvg from '@/assets/common/ic_point.svg';
import finishedPointSvg from '@/assets/common/ic_finishpoint.svg';
...
window.mapControl = {
    ...
    */
     ...
     *
     */
    initStorePointOverlay() {
        this.storePointOverlay = function (point,handovered) {
            this._point = point;
            this.type = 'storePoint';
            this.handovered=handovered;
        };
        this.storePointOverlay.prototype = new BMap.Overlay();
        this.storePointOverlay.prototype.initialize = function (map) {
            let that = this;
            this._map = map;
            let div = this._div = document.createElement('div');
            div.className = this.type;
            ...
            if(this.handovered==true){
                div.style.backgroundImage = `url(${finishedPointSvg})`;
                div.style.height = '45px';
                div.style.width = '45px';
            }else{
                div.style.backgroundImage = `url(${pointSvg})`;
        ...
        this.storePointOverlay.prototype.draw = function () {
            let map = this._map;
            let pixel = map.pointToOverlayPixel(this._point);
            this._div.style.left = pixel.x - 8 + 'px';
            this._div.style.top  = pixel.y  - 8 + 'px';
     ...
     *
     * @param {Object} point 点
     * @param {boolean} handovered 已交接
     */
    addStorePointOverlay(point,handovered) {
        let myCompOverlay = new this.storePointOverlay(point, handovered);
        map.addOverlay(myCompOverlay);
    },
    ...
    *
    */
    removeStorePointOverlay() {
        let overlays = map.getOverlays();
        let length = overlays.length;
        let storePointOverlays = [];
        for (let i = 0; i < length; i++) {
            if (overlays[i].type =='storePoint') {
                storePointOverlays.push(overlays[i]);
            }
        }
        for (let j = 0; j < storePointOverlays.length; j++) {
            map.removeOverlay(storePointOverlays[j]);
        }
    },
