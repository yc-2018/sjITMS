...
var EntityStore = Reflux.createStore({
    ...
    onList: function(pageindex) {
        ...
        var params = {
            ...
            'page_size': 10,
            //'filter':'task_date:' + Commonfun.getCurrentDate(),
            'filter':'entity_names:',
        };
        let queryParams={
            // page:pageindex,
        ...
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            if (response && response.success && response.data && response.data.records) {
               ...
               //params.filter=params.filter+'|entity_names:'+entity_names;
            ...
            Urls.jsonp(Urls.searchEntity, params, function(data) {
                if (data.status === 0) {
                    ...
                    that.trigger('total', that.data.total);
                    that.trigger('totalpage', Math.ceil(that.data.total / 15));
                } else {
                    that.trigger('list', []);
                    that.trigger('listcomplete', 0);
                    that.trigger('total', 0);
                    that.trigger('totalpage', 0);
                    that.trigger('initpage');
                }
            }.bind(that));
        });
