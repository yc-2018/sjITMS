...
class Trackcontent extends React.Component {
    ...
    onStatusChange = (type,data) => {
        switch (type){
            ...
            case 'triggerselecttrack':
                ...
                break;
            case 'trackStorePointer':
                this.listenTrackStorePointer(data);
            break;
    ...
    /**
     * 监听Store trackStorePointer事件，绘制门店位置
     *
     * @param {Object} data 门店坐标
     */
    listenTrackStorePointer = (data) => {
        this.addStorePoint(data);
    }
    /**
     * 显示某条线路上的所有门店
     *
     */
    addStorePoint = (data,handovered) => {
        mapControl.removeStorePointOverlay();
        for(var i = 0; i<data.length; i++){
            mapControl.addStorePointOverlay(data[i].point,data[i].handovered);
        }
    }
