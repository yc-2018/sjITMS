...
var TrackStore = Reflux.createStore({
    ...
    data: {
        ...
        // 实时监控查询日期
        queryEntityDate: formatDate(new Date(),true),//当前时间
        // 轨迹查询查询日期
        queryTrackDate: Commonfun.getCurrentDate(),
        // 实时监控可查看的entityNames
        entityNames: '',
        // 轨迹查询可查看的entityNames
        trackNames: ''
    ...
    onGetservicename: function() {
        this.trigger('servicename', '配送助手');
        // 车辆行业0，
        this.trigger('servicetype', 1);
    },
    /**
    ...
    onSearchallentity: function(index) {
        ...
        let queryParams={
            ...
            // pageSize:10,
            searchKeyValues:{
                'startTime':that.data.queryEntityDate
            }
        }
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            if (response && response.success && response.data && response.data.records) {
               let entity_names='';
               for (let n=0;n<response.data.records.length;n++) {
                   if(response.data.records[n].state=='DELIVERYING' || response.data.records[n].state=='FINISHED'){
                        entity_names = entity_names + response.data.records[n].billNumber;
                        if (n < response.data.records.length - 1) {
                            entity_names = entity_names + ',';
                        }
                   }
               }
               params.filter=params.filter+entity_names;
            }else{
                return;
            }
    ...
    onSearchofflineentity: function(index) {
        ...
        let queryParams={
            ...
            searchKeyValues:{
                'startTime':that.data.queryEntityDate,
                'state':'FINISHED',
                'billNumber':that.data.searchQuery,
            }
        }
        var params = {
            'query': that.data.searchQuery,
        ...
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            if (response && response.success && response.data && response.data.records) {
               ...
               params.filter='entity_names:'+entity_names;
            }else{
                return ;
            }
            Urls.jsonp(Urls.searchEntity, params, function(data) {
                if (data.status === 0) {
                    ...
                    that.trigger('totaloffline', data.total);
                    that.trigger('totalofflinepage', Math.ceil(data.total / 10));
                    that.data.searchingOffline = 0;
                } else {
                    that.setOfflineEntities([]);
                    that.setOfflineCompleteEntities([]);
                    that.trigger('totaloffline', 0);
                    that.trigger('totalofflinepage', 0);
                    that.trigger('initofflinepage');
                    that.data.searchingOffline = 0;
                }
            }.bind(that));
        })
    ...
    onSearchonlineentity: function(index) {
        ...
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            if (response && response.success && response.data && response.data.records) {
               let entity_names='';
               for (let n=0;n<response.data.records.length;n++) {
                    if(response.data.records[n].state=='DELIVERYING'){
                        entity_names = entity_names + response.data.records[n].billNumber;
                        if (n < response.data.records.length - 1) {
                            entity_names = entity_names + ',';
                        }
            ...
            Urls.jsonp(Urls.searchEntity, params, function(data) {
                if (data.status === 0) {
                    ...
                    that.trigger('totalonline', data.total);
                    that.trigger('totalonlinepage', Math.ceil(data.total / 10));
                    that.data.searchingOnline = 0;
                } else {
                    that.setOnlineEntities([]);
                    that.setonlineCompleteEntities([]);
                    that.trigger('totalonline', 0);
                    that.trigger('totalonlinepage', 0);
                    that.trigger('initonlinepage');
                    that.data.searchingOnline = 0;
                }
            }.bind(that));
        })
    ...
    onBoundsearchentity(display) {
        ...
        let queryParams={
            ...
            // pageSize:1000,
            searchKeyValues:{
                'startTime':formatDate(new Date(),true),
            }
        }
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            that.data.entityNames='';
            if (response && response.success && response.data && response.data.records) {
               for (let n=0;n<response.data.records.length;n++) {
                    if(response.data.records[n].state=='DELIVERYING' || response.data.records[n].state=='FINISHED'){
                        that.data.entityNames = that.data.entityNames + response.data.records[n].billNumber;
                        if (n < response.data.records.length - 1) {
                            that.data.entityNames = that.data.entityNames + ',';
                        }
                    }
               }
            }
            that.boundsearchentity(display);
        })
    },

    boundsearchentity(display) {
        let that = this;
        let time = new Date();
        ...
        if (this.data.entityNames.length > 0) {
            filter = filter.length>0 ? '|entity_names:' + this.data.entityNames : 'entity_names:' + this.data.entityNames;
        }
                    ...
                    if (item.entity_name === entity_name) {
                        ...
                        Urls.jsonp(Urls.getAddress, paramsGeo, function(dataGeo) {
                            ...
                            if (dataGeo.result.formatted_address !== '') {
                                ...
                                address = dataGeo.result.addressComponent.city;
                            }
    ...
    onTracklist: function(index) {
        ...
        let queryParams={
            ...
            // pageSize:10,
            searchKeyValues:{
                'startTime':formatDate(that.data.queryTrackDate,true),
                'billNumber':that.data.searchQueryTrack
            }
        }
        Urls.query(Urls.getEntityNames,queryParams,function(response){
            that.data.trackNames='';
            if (response && response.success && response.data && response.data.records) {
               for (let n=0;n<response.data.records.length;n++) {
                that.data.trackNames = that.data.trackNames + response.data.records[n].billNumber;
                    if (n < response.data.records.length - 1) {
                        that.data.trackNames = that.data.trackNames + ',';
                    }
               }
            }else{
                return;
            }
            that.trackList(index);
        })
    },

    trackList: function(index) {
        ...
        var params = {
            ...
            'page_size': 10,
            'page_index': index,
            //filter:'task_date'+that.data.queryTrackDate,
        };
        if (this.data.trackNames.length > 0) {
            params.filter = 'entity_names:' + this.data.trackNames;
        }
        Urls.jsonp(Urls.searchEntity, params, function(data) {
            if(data.status === 0) {
    ...
    * @param {string} data entity数据
    ...
    onChangedatetime: function(data) {
        ...
        this.data.queryTrackDate=Commonfun.getStartTime(date);
    ...
    onSelecttrack: function(data) {
        ...
        let queryParam={
            shipPlanBillNumber:that.data.selectTrack
        }
        var partTime = Math.floor((that.data.end_time - that.data.start_time) / 6);
        for(var i = 0; i < 6; i++) {
            tempTimeArr[i] = {
                ...
                Urls.jsonp(Urls.getTrack, paramsearch, function(data) {
                    ...
                    if(data.status === 0){ 
                        ...
                        if(that.data.trackRouteDataCount === 12) {
                            ...
                            if(that.data.trackRouteNoZero.length>0){
                                Urls.get(Urls.getTrackStorePoint,queryParam, function(response) {
                                    let data=[];
                                    if(response && response.success && response.data){
                                        if(response.data.handoveredStorePoints){
                                            let handoveredpoints=response.data.handoveredStorePoints;
                                            for(let i=0;i<handoveredpoints.length;i++){
                                                let splitdata=handoveredpoints[i].split(';');
                                                data.push({point:new BMap.Point(splitdata[1],splitdata[0]),handovered:true});
                                            } 
                                        }
                                        if(response.data.unHandoverStorePoints){
                                            let unhandoveredpoints=response.data.unHandoverStorePoints;
                                            for(let i=0;i<unhandoveredpoints.length;i++){
                                                let splitpoints=unhandoveredpoints[i].split(';');
                                                data.push({point:new BMap.Point(splitpoints[1],splitpoints[0]),handovered:false});
                                            }
                                        }
                                    }
                                    that.trigger('trackStorePointer', data);
                                })
                            }
