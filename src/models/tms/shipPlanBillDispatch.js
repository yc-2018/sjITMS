import {
  ...
  shipRollback,
  onApprove,
  getShipPlanByBillNumber,
  getMerge,
  adjust,
  queryScheduleBill,
  getDeliveryScheduleMerge,
  getOnlyBillScheduleMerge,
  getTakeDeliveryScheduleMerge,
} from '@/services/tms/ShipPlanBillDispatch';

export default {
  namespace: 'shipPlanBillDispatch',
  subscriptions: {
    /**
     * 监听浏览器地址
    ...
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if(location.payload && location.pathname == "/tms/shipPlanBillDispatch"){
          dispatch({
            type: 'showPage',
  ...
  effects: {
    ...
    *getDeliveryScheduleMerge({ payload,callback }, { call, put }) {
      const response = yield call(getDeliveryScheduleMerge, payload);
      if (response && response.success) {
        yield put({
          type: 'onView',
          payload: response.data
    ...
    *getOnlyBillScheduleMerge({ payload,callback }, { call, put }) {
      const response = yield call(getOnlyBillScheduleMerge, payload);
      if (response && response.success) {
        yield put({
          type: 'onView',
          payload: response.data
    ...
    *getTakeDeliveryScheduleMerge ({ payload,callback }, { call, put }) {
      const response = yield call(getTakeDeliveryScheduleMerge, payload);
      if (callback) callback(response);
    },
